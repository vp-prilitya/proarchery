...
<div class="container-fluid animatedParent animateOnce my-3">
        <div class="animated fadeInUpShort">
            ...
            <div class="row">
               <div class="col-sm-12">
                  <div class="white shadow r-15 mb-3 no-b">
                     <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div class="align-self-center">
                                <ul class="nav nav-pills mb-3" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link active show r-20" id="w3--tab1" data-toggle="tab" href="#w3-tab1" role="tab" aria-controls="tab1" aria-expanded="true" aria-selected="true">Toko</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link r-20" id="w3--tab2" data-toggle="tab" href="#w3-tab2" role="tab" aria-controls="tab2" aria-selected="false">Jasa</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="align-self-center">
            ...
            <div class="row">
               ...
               <div class="tab-content">
                        ...
                        <div class="white shadow r-15 mb-3 r-15 no-b shadow">
                           <div class="card-body">
                              <div class="form-group">
                                 <input type="text" name="search" id="search" placeholder="Cari Item Produk Toko" class="form-control" />
                        ...
                        <div class="row">
                           ...
                           <div class="col-sm-3">
                              <div class="white shadow r-15 text-center p-4 mb-3 card__hover shadow">
                                 ...
                                 <button type="button" data-id='<?=$db['id']?>' data-raw='<?=$db['need_raw']?>' data-paket='<?=$db['is_paket']?>'  data-item='<?=$db['nama']?> | <?=$db['satuan']?> | Stok <?=$db['stok']?> | Harga <?=$db['harga_jual']?>' data-jenis_barang='toko' class="btn btn-success btn-sm r-10 mt-2 tombol-add"><i class="icon-plus"></i> Add</button>
                              </div>
                           </div>
                           <?php endforeach?>
                        </div>
                  ...
                  <div class="tab-pane fade" id="w3-tab2" role="tabpanel" aria-labelledby="w3-tab2">
                     <div class="white mb-3 r-15 no-b shadow mt-0">
                           <div class="card-body">
                              <div class="form-group">
                                 <input type="text" name="searchJasa" id="searchJasa" placeholder="Cari Item Produk Jasa" class="form-control" />
                                 <ul class="list-group" id="resultJasa"></ul>
                              </div>
                           </div>
                     </div>

                     <div class="row">
                        <?php foreach($jasa as $db):?>
                        <div class="col-sm-3">
                           <div class="white shadow r-15 text-center p-4 mb-3 card__hover shadow">
                              <!-- <i class="icon-data_usage s-48"></i> -->
                              <?php if($db['poto'] == null) :?>
                              ...
                              <?php endif?>
                              <h6 class="mt-3"><?=$db['nama']?></h6>
                              <button type="button" data-id='<?=$db['id']?>' data-raw='<?=$db['need_raw']?>' data-paket='<?=$db['is_paket']?>'  data-item='<?=$db['nama']?> | <?=$db['satuan']?> | Stok <?=$db['stok']?> | Harga <?=$db['harga_jual']?>' data-jenis_barang='jasa' class="btn btn-success btn-sm r-10 mt-2 tombol-add"><i class="icon-plus"></i> Add</button>
                           </div>
                        </div>
                        <?php endforeach?>
                     </div>
               ...
               <div class="col-sm-6">
               <form action="<?=base_url('penjualan/save')?>" method="post" target="_blank" id="form">
                  <input type="hidden" name="is_jasa" id="is_jasa" value="toko" required>
                     ...
                     <div class="card-body">
                        <div class="table-responsive">
                           <table class="table">
                              ...
                              <tr>
                                 <th>DISKON (%) : </th>
                                 <td><input type="number" name="diskon" id="diskon" class="form-control" onkeyup="mydiskon(this.value)" value=0 max=100 /></td>
                              </tr>
                              <tr>
                                 <th>DISKON (IDR) : </th>
                                 <td>
                              ...
                              <!--   <th>PPN 10% (IDR) : </th>-->
                              <!--   <td>-->
                                     <input type="hidden" name="ppn" id="ppn" class="form-control" readonly value=0 />
                                     <!--</td>-->
                              <!--</tr>-->
                              <tr>
                                 <th>TOTAL PEMBELANJAAN (IDR) : </th>
                                 <td>
                                     <input type="text" name="sub_total_2" id="sub_total_2" class="form-control" readonly value=0 />
                                     <input type="hidden" name="total_tagihan2" id="total_tagihan2" class="form-control" readonly value=0 />
                                 </td>
                              </tr>
                              <tr>
...
function _subtotal() {
   ...
   $('input[name="jenis_barang[]"]').each(function() {
      jenis.push(this.value);
   });

   $('#total_gross').val(rupiah(total));
  $('#total_tagihan').val(rupiah(total));
   $('#sub_total_2').val(rupiah(total));

   jenis.includes("jasa") == true ? $('#is_jasa').val('jasa') : $('#is_jasa').val('toko');
...
function totalBayar(value) {
   var value = value.replace(/\./g,'');
//   var total_tagihan = $('#total_tagihan2').val().replace(/\./g,'');
...
$(document).ready(function(){
 ...
 $('#search').keyup(function(){
   ...
   if(cari != ''){
      $.ajax({
         ...
         type:"POST",
         dataType: 'json',
         data:{cari:cari, jenis_barang:jenis_barang},
         success:function(data){
 ...
 $('#result').on('click', 'li', function() {
   ...
   showTable(id, nama, raw, paket, item, jenis_barang);
 });

 $('#searchJasa').keyup(function(){
   $('#resultJasa').html('');
   var html = ``;
   var cari = $('#searchJasa').val();
   var jenis_barang = 'jasa';
   ...
   if(cari != ''){
      $.ajax({
         ...
         success:function(data){
            ...
            $('#resultJasa').append(html);
         }
      });
   }
 });
 
 $('#resultJasa').on('click', 'li', function() {
   var item = $(this).text().split('|');
   var nama = item[0];
   var id = $(this).data('id');
   var paket = $(this).data('paket');
 ...
 function showTable(id, nama, raw, paket, item, jenis_barang) {
   var item = item;
   var nama = nama;
   var aksi = `<button type="button" class="btn btn-danger btn-xs delete-row"><i class="icon-delete_forever"></i></button>`;
   
   if(raw == 1 && paket == 0){
      var quantity = `<input type="number" class="form-control" id="quantityRaw`+id+`" name="quantityRaw[]" required onchange="quantity('Raw`+id+`')" value=1 />
      ...
      var disc = `<input type="number" max=100 class="form-control" id="discRaw`+id+`" name="discRaw[]" required onkeyup="diskonItem('Raw`+id+`')" value=0 >
                  <input type="hidden" class="form-control" id="jenis_barangRaw`+id+`" name="jenis_barang[]" required value=`+jenis_barang+` />`;
   } 

   if(raw == 0 && paket == 0){
      var quantity = `<input type="number" class="form-control" id="quantity`+id+`" name="quantity[]" required onchange="quantity(`+id+`)" value=1 />
      ...
      var disc = `<input type="number" max=100 class="form-control" id="disc`+id+`" name="disc[]" required onkeyup="diskonItem(`+id+`)" value=0 >
                  <input type="hidden" class="form-control" id="jenis_barang`+id+`" name="jenis_barang[]" required value=`+jenis_barang+` />`;
   }

   if(raw == 0 && paket == 1){
      var quantity = `<input type="number" class="form-control" id="quantity`+id+`" data-paket=1 name="quantity[]" required onchange="quantity(`+id+`)" value=1 />
                  <input type="hidden" class="form-control" id="id`+id+`" name="id[]" required value="`+id+`"/>
                  <input type="hidden" class="form-control" id="harga`+id+`" name="harga[]" required value="`+item[3].replace(" Harga ", "")+`"/>
                  <input type="hidden" class="form-control" id="subtotal`+id+`" name="subtotal[]" required value=`+item[3].replace(" Harga ", "")+` />
