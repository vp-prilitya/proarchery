                  ...
                  <tbody>
                     <?php $i=1; foreach($data as $db):?>
                        ...
                        <tr style="<?=$style?>">
                           <td><?=$i++?></td>
                           <td><?=$db['nama']?></td>
                           <td><?=number_format($db['total']??$db['hutang'],0,'',',')?></td>
                           <td><?=number_format($db['bayar']??0,0,'',',')?></td>
                           <td><?=number_format(($db['total']??$db['hutang']) - $db['bayar']??0,0,'',',')?></td>
                           <!-- <td><button type="button" data-remote="<?=base_url('hutang/detailHutang/')?><?=$db['vendor_id']?>" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#exampleModal"><i class="icon-plus"></i> Detail</button></td> -->
                           <td><button type="button" data-remote="<?=base_url('hutang/detailHutang2/')?><?=$db['vendor_id']?>" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#exampleModal"><i class="icon-plus"></i> Detail</button></td>
                        </tr>
                     <?php endforeach?>
                  </tbody>
               </table>
