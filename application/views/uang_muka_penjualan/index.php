        ...
        <div class="row ml-1">
         ...
         <div class="white shadow r-15">
            ...
            <div class="row">
               ...
               <div class="col">
                  <a href="<?=base_url('uang_muka_penjualan/create')?>" class="btn btn-primary btn-xs float-right mt-5 mr-5"><i class="icon-plus"></i> Tambah</a>
               </div>
            <!-- </div> -->
            </div>
            <div class="card-body">
                  ...
                  <tbody>
                     <?php $i=1; foreach($data as $db):?>
                        <tr>
                           <td><?=$i++?></td>
                           <td><?=$db['nama']?></td>
                           <td><?=number_format($db['jumlah'],0,'','.')?></td>
                           <td><?=ucfirst($db['jenis_ppn'])?></td>
                           <td><?=number_format(floatval($db['ppn']),0,'','.')?></td>
                           <td><?=$db['akun']?></td>
                           <td><?=$db['catatan']?></td>
                           <td><?=$db['created']?></td>
                           <td>
                              <a href="<?=base_url('uang_muka_penjualan/edit/')?><?=$db['id']?>" class="btn btn-warning btn-xs"><i class="icon-edit"></i> Ubah</a>
                              <a href="<?=base_url('uang_muka_penjualan/delete/')?><?=$db['id']?>" class="btn btn-danger btn-xs tombol-hapus" ><i class="icon-delete_forever"></i> Hapus</a>
                           </td>
                        </tr>
                     <?php endforeach?>
                  </tbody>
