            ...
            <div class="row">
               <div class="col-sm-6">
                  <div class="white shadow r-15">
                     ...
                     <form action="<?=base_url('uang_muka_penjualan/update')?>" method="post">
                     <input type="hidden" id="id" name="id" value="<?=$data['id']?>" required>
                        <div class="form-group">
                           ...
                           <select class="custom-select select2" name="pelanggan_id" id="pelanggan_id" required>
                              ...
                              <?php foreach($pelanggan as $db):?>
                                 <option value="<?=$db['id']?>" <?=$data['pelanggan_id']==$db['id']?'selected':''?>><?=$db['nama']?> || <?=$db['contact']?></option>
                              <?php endforeach?>
                           </select>
                        </div>

                        <?php if($data['jenis_ppn']=='exclude'){
                                 $jml = $data['jumlah']-$data['ppn'];
                              } else {
                                 $jml = $data['jumlah'];
                              }?>
                        ...
                        <div class="form-group">
                           ...
                           <input type="text" class="form-control" placeholder="Masukan Jumlah" id="jumlah2" name="jumlah2" required onkeyup="jumlahBayar(this.value)" value="<?=number_format($jml,0,'','.')?>">
                        </div>
                        ...
                        <div class="form-group">
                           ...
                           <select class="custom-select select2" name="jenis_ppn" id="jenis_ppn" required>
                              ...
                              <option value="exclude" <?=$data['jenis_ppn']=='exclude'?'selected':''?>>Exclude PPN</option>
                              <option value="include" <?=$data['jenis_ppn']=='include'?'selected':''?>>Include PPN</option>
                           </select>
                        </div>

                        <div class="form-group">
                           <label for="ppn">Pajak</label>
                           <input type="text" class="form-control" placeholder="Masukan ppn" id="ppn" name="ppn" required readonly value="<?=number_format($data['ppn'],0,'','.')?>">
                        </div>

                        <div class="form-group">
                           <input type="hidden" class="form-control"id="jumlah" name="jumlah" required readonly value="<?=$jml?>">
                        </div>
                        ...
                        <div class="form-group">
                           ...
                           <select class="custom-select select2" name="akun_id" id="akun_id" required>
                              ...
                              <?php foreach($akun as $db):?>
                                 <option value="<?=$db['id']?>" <?=$data['akun_id']==$db['id']?'selected':''?>><?=$db['kode']?> - <?=$db['nama']?></option>
                              <?php endforeach?>
                           </select>
                        </div>
                        ...
                        <div class="form-group">
                           ...
                           <textarea class="form-control" placeholder="Masukan Memo" id="catatan" name="catatan" required><?=$data['catatan']?></textarea>
                        </div>
                        
                        <a href="<?=base_url('uang_muka_penjualan')?>" class="btn btn-danger mr-2">Batal</a>
