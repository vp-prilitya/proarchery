<div class="container-fluid animatedParent animateOnce my-3">
        ...
        <div class="row ml-1">
            ...
            <form action="<?=base_url('barang_jual/updatePaket')?>" method="post" enctype="multipart/form-data">
            <div class="row mb-3">
               <div class="col-sm-12">
                     ...
                     <div class="card-body">
                        ...
                        <div class="row">
                           ...
                           <div class="col-sm-6">
                              ...
                              <div class="form-group">
                                 <label for="harga_jual">Gross Profit</label>
                              ...
                              <!-- <div class="row">
                                 ...
                                 <div class="col-sm mt-4">
                                    <div class="material-switch mt-1" style="margin-left:-60px;">
                                       <input id="jadwal" name="jadwal" type="checkbox" value="1" <?=$data[0]['jadwal'] == 1 ? 'checked':''?> />
                                       <label for="jadwal" class="bg-primary"></label>
                                    </div>
                                 </div>
                              </div> -->
            ...
            <div class="row mb-3">
               <div class="col-sm-12">
                  <div class="white shadow r-15">
                     <div class="card-title gradient p-3 r-15" ><h5 class="text-white text-bold"><strong>Tambah Produk</strong></h5></div>
                     <div class="card-body">
                        <div class="form-group">
                           <label for="barang_jual_id">Barang Jual</label>
                        ...
                        <div class="table-responsive">
                           <table id="example2" class="table table-bordered table-striped table-hover data-tables" data-options='{"paging":false}'>
                              <thead>
                                 <tr>
                                    <th>Nama</th>
                                    <th>Satuan</th>
                              ...
                              <tbody>
                                 <?php if($data[0]['need_raw']==0 AND $data[0]['is_paket']==1):?>
                                    <?php $total = 0; foreach($data as $db) : ?>
                                       <?php $total += $db['harga_pokok'] * $db['quantity']?>
                                    <tr>
                                       <td><?=$db['paket']?></td>
                                       <td><?=$db['p_satuan']?></td>
                                       <td>
                                          <input type="text" class="form-control" id="hargaBarangJualSatuan<?=$db['barang_id']?>" name="hargaBarangJualSatuan[]" required value="<?=number_format($db['harga_pokok'],0,'','.')?>" readonly>
                                          <input type="hidden" class="form-control" id="hargaBarangJual<?=$db['barang_id']?>" name="hargaBarangJual[]" required value="<?=$db['harga_pokok'] * $db['quantity']?>">
                                       </td>
                                       <td>
                                          <input type="text" class="form-control" id="quantityBarangJual<?=$db['barang_id']?>" name="quantityBarangJual[]" required value="<?=$db['quantity']?>" onkeyup="qty(<?=$db['barang_id']?>, this.value)">
                                          <input type="hidden" class="form-control" id="idBarangJual<?=$db['barang_id']?>" name="idBarangJual[]" required value="<?=$db['barang_id']?>">
                                       </td>
                                       <td>
                                          <button type="button" class="btn btn-danger btn-xs delete-row2"><i class="icon-delete_forever"></i></button>
                                       </td>
                                    </tr>
                                    <?php endforeach?>
                                 <?php endif?>
                        ...
                        <div class="text-right">
                           <h3 class="text-dark"><strong>Total : <span id="total_table"><?=number_format($total,0,'','.')?></span></strong></h3>
               ...
               <div class="col-sm-12">
                  <div class="white shadow r-15">
                     <div class="card-body text-right">
                        <a href="<?=base_url('barang_jual/paket')?>" class="btn btn-danger mr-2 mt-5">Batal</a>
...
function _harga_pokok() {
   ...
   var total = 0;
   $('input[name="hargaBarangJual[]"]').each(function() {
...
function qty(id, value) {
   var hrg = $('#hargaBarangJualSatuan'+id).val().replace(/\./g,'') * value;
   $('#hargaBarangJual'+id).val(hrg);
   _harga_pokok();
}
</script>
<script>
      ...
      $( "#barang_jual_id" ).change(function() {
         ...
         $.ajax({
                ...
                success:function(data){
                     var quantity = `<input type="text" class="form-control" id="quantityBarangJual`+data.id+`" name="quantityBarangJual[]" required onkeyup="qty(`+data.id+`, this.value)" value="1">
                                    <input type="hidden" class="form-control" id="idBarangJual`+data.id+`" name="idBarangJual[]" required value="`+data.id+`">`;

                     var hrg = data.harga_pokok == ''? 0 : data.harga_pokok;
                     var harga = `<input type="text" class="form-control" id="hargaBarangJualSatuan`+data.id+`" name="hargaBarangJualSatuan[]" required value="`+rupiah(hrg)+`" readonly>
                              <input type="hidden" class="form-control" id="hargaBarangJual`+data.id+`" name="hargaBarangJual[]" required value="`+hrg+`">`;
