<div class="container-fluid animatedParent animateOnce my-3">
        ...
        <form action="<?=base_url('retur_penjualan/save')?>" method="post">
               ...
               <div class="card-body">
                  <div class="form-group">
                     <select class="custom-select select2" name="search_select" id="search_select">
                        <option value="" disabled selected>Pilih Surat Jalan (SJ)</option>
                        <option value="cari_input">Cari Surat Jalan (SJ)</option>
                        <?php foreach($sj as $db):?>
                           <option value="<?=$db['no_faktur']?>"><?=$db['no_faktur']?></option>
                        <?php endforeach?>
                     </select>
                     <input type="text" name="search" id="search" placeholder="Cari Nomor Surat Jalan" class="form-control hilang" required />
                     <input type="hidden" name="pengiriman_penjualan_id" id="pengiriman_penjualan_id" class="form-control" required/>
               ...
               <div class="col-sm-6">
                  <div class="white shadow r-15 mb-3">
                     <div class="card-body">
                        ...
                        <!-- <a href="<?=base_url('retur_penjualan/create')?>" class="btn btn-success mr-2 float-right">Buat Baru</a> -->
                        <a href="<?=base_url('retur_penjualan')?>" class="btn btn-danger mr-2 float-right">Batal</a>
...
$(document).ready(function(){
 ...
 $('.cari').click(function(){
   ...
   if(po != ''){
      $.ajax({
         url:"<?= base_url('retur_penjualan/getSJ')?>",
         type:"POST",
         dataType: 'json',
         data:{po:po},
         success:function(data){
            $('#pengiriman_penjualan_id').val(data[0].pengiriman_penjualan_id)
 ...
 function showTable(id, nama, raw, paket, harga, qty_po, qty_kirim) {
   ...
   var quantity = `<input type="number" class="form-control" id="quantity`+id+`" name="quantity[]" required onkeyup="quantity(`+id+`)" value=1 min=1 max=`+qty_kirim+` data-paket=`+paket+` />
   ...
   if(raw == 0 && paket == 1){
      quantity += `<input type="hidden" class="form-control" id="quantityPaket`+id+`" name="quantityPaket`+id+`" required value=1 />`;
   }
