         ...
         <div class="white shadow r-15">
            ...
            <div class="card-body">
               ...
               <table id="example2" class="table table-bordered table-striped table-hover">
                  ...
                  <tbody>
                     <?php $i=1; $total=0; foreach($data as $db): $total += $db['harga_beli'] * $db['qty']?>
                        <tr>
                           <td><?=$i++?></td>
                           <td><?=$db['nama']??$db['mentah']?></td>
                           <td><?=$db['qty']?></td>
                           <td><?=$db['satuan']??$db['satuan_mentah']?></td>
                           <td><?=number_format($db['harga_beli'],0,'',',')?></td>
                           <td><?=number_format($db['harga_beli'] * $db['qty'],0,'',',')?></td>
                        </tr>
                     <?php endforeach?>
                     <tr>
                        <td></td>
   ...
   var sampai = <?=json_encode($sampai)?>;
    $(document).ready(function() {
        ...
        var buttons2 = new $.fn.dataTable.Buttons(table2, {
            buttons: [{
                extend: 'excelHtml5',
                title: 'Laporan Pembelian ' + dari + ' s.d ' + sampai,
                text: 'EXCEL',
