<div class="container-fluid animatedParent animateOnce my-3">
        ...
        <form action="<?=base_url('pesanan_penjualan/save')?>" method="post">
            ...
            <div class="white shadow r-15 mb-3">
               <div class="card-body">
                  <div class="form-group">
                     <label for="barang_jual_id">Pilih Barang</label>
                     <select class="custom-select select2" name="barang_jual_id" id="barang_jual_id">
                     <option value="" disabled selected>Pilih Barang</option>
                     <?php foreach($produk as $db):?>
                        <option value="<?=$db['id']?>" data-raw=0 data-paket=><?=$db['nama']?> - <?=$db['no_part']?> | <?=$db['satuan']?> | Stok <?=$db['stok']?> | Harga <?=number_format($db['harga_pokok']==''?0:$db['harga_pokok'],0,'','.')?></option>
                     <?php endforeach?>
                     <?php foreach($paket as $db):?>
                        <option value="<?=$db['id']?>" data-raw=0 data-paket=1><?=$db['nama']?> - <?=$db['no_part']?> | <?=$db['satuan']?> | Stok <?=$db['stok']?> | Harga <?=number_format($db['harga_pokok'],0,'','.')?></option>
                     <?php endforeach?>
                  </select>
                  </div>
               </div>
                        ...
                        <div class="table-responsive">
                           <table class="table">
                              ...
                              <tr>
                                 ...
                                 <td>
                                 <select class="custom-select select2" name="bank_id" id="bank_id" required>
                                    <option value="" disabled selected>Pilih Bank</option>
                                    <?php foreach($bank as $db):?>
                                       <option value="<?=$db['id']?>"><?=$db['nama']?> || <?=$db['no_rek']?></option>
                                    <?php endforeach?>
                                 </select>
                                 </td>
                              </tr>
