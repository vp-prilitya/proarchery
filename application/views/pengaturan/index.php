        ...
        <div class="row ml-1">
         ...
         <div class="white shadow r-15">
            ...
            <div class="row">
               ...
               <div class="col">
                  <a href="<?=base_url('pengaturan/create')?>" class="btn btn-primary btn-xs float-right mt-5 mr-5"><i class="icon-plus"></i> Tambah</a>
               </div>
            </div>
            <!-- </div> -->
            <div class="card-body">
               ...
               <table id="example2" class="table table-bordered table-striped table-hover data-tables" data-options='{ "paging": false; "searching":false}'>
                  ...
                  <tbody>
                     <?php $i=1; foreach($data as $db):?>
                        <tr>
                           <td><?=$i++?></td>
                           <td><?=(ucfirst($db['variable'])=='Penjualan' OR ucfirst($db['variable'])=='Pembelian')? ucfirst($db['variable']) : 'Pembayaran '.ucfirst($db['variable'])?></td>
                           <td><?=ucfirst($db['jenis'])?></td>
                           <td><?=$db['nama']?></td>
                           <td><?=$db['is_debit']==1?'Debit':'Kredit'?></td>
                           <td><?=$db['kelompok']?></td>
                           <td>
                              <a href="<?=base_url('pengaturan/edit/')?><?=$db['id']?>" class="btn btn-warning btn-xs"><i class="icon-edit"></i> Ubah</a>
                              <a href="<?=base_url('pengaturan/delete/')?><?=$db['id']?>" class="btn btn-danger btn-xs tombol-hapus" ><i class="icon-delete_forever"></i> Hapus</a>
                           </td>
                        </tr>
                     <?php endforeach?>
                  </tbody>
